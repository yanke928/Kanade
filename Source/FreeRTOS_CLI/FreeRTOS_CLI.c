...
#define COMMAND_LINE_PRIORITY tskIDLE_PRIORITY+10
...
void vCommandConsoleTask(void *pvParameters)
{
	...
	USBCDC_SendString(pcWelcomeMessage);

	for (;;)
	{
		/* This implementation reads a single character at a time.  Wait in the
		Blocked state until a character is received. */
	rereceive:tmp = USBCDC_getc(&cRxedChar);
		if (tmp == 0)
		{
		...
		for (;;)
		{
			tmp = USBCDC_getc(&cRxedChar);
			if (tmp == 0) goto rereceive;
		DealWithData:
			if( cInputIndex > 0 || cRxedChar!='\b')
			USBCDC_putc(cRxedChar);
			if (cRxedChar == '\b')
			{
				if (cInputIndex > 0)
				{
		...
		do
		{
			...
			USBCDC_SendString((char *)pcOutputString);
...
{
	xTaskCreate(vCommandConsoleTask, "CommandLine",
		256, NULL, COMMAND_LINE_PRIORITY, NULL);
	RegisterCommands();
}
