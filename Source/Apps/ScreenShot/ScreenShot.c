...
void TakeAScreenShot()
{
  ...
  FIL bmp;
  char tempString[20];
  sprintf(tempString,"0:/%d.bmp",ScreenShotCount);
  ScreenShotCount++;
	res = f_open(&bmp, tempString, FA_CREATE_ALWAYS | FA_WRITE);
	if (res != FR_OK) ShowDetailedFault("Create screenshot file failed");
  if (WriteOLED12864BitMapHeader(&bmp) != true)
  ShowDetailedFault("Write screenshot file header failed");
  if (WriteOLED12864BitMap(&bmp) != true)
  ShowDetailedFault("Write screenshot file content failed");
  f_close(&bmp);
}
